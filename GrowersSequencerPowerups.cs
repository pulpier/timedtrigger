using GrowersAstro.NINA.GrowersSequencerPowerups.Properties;
...
using Settings = GrowersAstro.NINA.GrowersSequencerPowerups.Properties.Settings;
...
namespace GrowersAstro.NINA.GrowersSequencerPowerups {
    ...
    /// The user interface for the settings will be defined by a DataTemplate with the key having the naming convention "GrowersSequencerPowerups_Options" where GrowersSequencerPowerups corresponds to the AssemblyTitle - In this template example it is found in the Options.xaml
    /// </summary>
    [Export(typeof(IPluginManifest))]
    public class GrowersSequencerPowerups : PluginBase, INotifyPropertyChanged {
        private readonly IPluginOptionsAccessor pluginSettings;
        private readonly IProfileService profileService;
        private readonly IImageSaveMediator imageSaveMediator;

        // Implementing a file pattern
        private readonly ImagePattern exampleImagePattern = new ImagePattern("$$EXAMPLEPATTERN$$", "An example of an image pattern implementation", "Growers Sequencer Powerups");

        [ImportingConstructor]
        public GrowersSequencerPowerups(IProfileService profileService, IOptionsVM options, IImageSaveMediator imageSaveMediator) {
